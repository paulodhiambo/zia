...
func (t *TokenManager) Token(ctx context.Context) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("fetch kcb token: %w", err)
	}
...
func (t *TokenManager) fetchToken(ctx context.Context) (string, int, error) {
	...
	req, err := http.NewRequestWithContext(ctx, "POST",
		baseURL+"/oauth/token?grant_type=client_credentials", nil)
	if err != nil {
		return "", 0, err
	}
	...
	if resp.StatusCode != http.StatusOK {
		return "", 0, fmt.Errorf("kcb auth failed: status=%d", resp.StatusCode)
	}
