...
func (r *ledgerRepo) InsertEntries(ctx context.Context, entries []domain.LedgerEntry) error {
	for _, e := range entries {
		_, err := r.db.Exec(ctx, `
			INSERT INTO ledger_entries (id, account_id, entry_type, amount_minor, currency,
				reference_type, reference_id, posted_at)
			VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`,
			e.ID, e.AccountID, e.EntryType, e.AmountMinor, e.Currency,
...
func (r *ledgerRepo) GetByAccount(ctx context.Context, accountID string, limit, offset int) ([]domain.LedgerEntry, error) {
	rows, err := r.db.Query(ctx, `
		SELECT id, account_id, entry_type, amount_minor, currency,
			reference_type, reference_id, posted_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&e.ID, &e.AccountID, &e.EntryType, &e.AmountMinor, &e.Currency,
